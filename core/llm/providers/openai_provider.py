...
class OpenAIProvider(LLMProvider):
    ...
    async def health(self) -> ProviderHealth:
        ...
        return ProviderHealth(
            ...
            available=True,
            model=self.model,
            message="OpenAI API key present. Live health not tested.",
        )

    async def generate(self, request: LLMRequest) -> LLMResponse:
        if not self.api_key:
            return LLMResponse(
                text="OpenAI provider is not configured. Set OPENAI_API_KEY.",
                provider=self.provider_name,
                model=self.model,
                success=False,
                error_type="missing_api_key",
        ...
        try:
            ...
            if not text:
                return self._failure("empty_response", "OpenAI returned an empty response.", start)
            return LLMResponse(
                text=text,
                provider=self.provider_name,
                model=self.model,
            ...
            return self._failure("timeout", "OpenAI request timed out.", start)
        except urllib.error.HTTPError as exc:
            return self._http_failure(exc, start)
        except urllib.error.URLError as exc:
            return self._failure("network_error", f"OpenAI network error: {exc.reason}", start)
        except json.JSONDecodeError:
            return self._failure("bad_json", "OpenAI returned invalid JSON.", start)
        except Exception as exc:
            return self._failure("exception", f"OpenAI request failed: {exc.__class__.__name__}", start)
    ...
    def _http_failure(self, exc: urllib.error.HTTPError, start: float) -> LLMResponse:
        ...
        if (
            ...
            message = "OpenAI API reported insufficient quota or billing access."
        elif exc.code in {401, 403}:
            error_type = "auth_error"
            message = "OpenAI API returned an authentication error."
        elif exc.code == 429:
            error_type = "rate_limit"
            message = "OpenAI rate limit reached."
        elif exc.code >= 500:
            error_type = "server_error"
            message = "OpenAI API returned a server error."
        else:
